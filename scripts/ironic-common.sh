...
{
    if [[ "$#" -ne 2 ]]; then
        echo "${FUNCNAME[0]}: two parameters required, $# provided" >&2
        return 1
    fi
    ...
    case $2 in
        ...
        6)
            QUERY="aaaa";;
        *)
            echo "${FUNCNAME[0]}: the second parameter should be <4|6> for A and AAAA records" >&2
            return 1;;
    esac
...
get_ip_of_interface()
{
    ...
    if [[ "$#" -gt 2 ]]; then
        echo "${FUNCNAME[0]}: too many parameters" >&2
        return 1
    fi
    ...
    if [[ "$#" -eq 2 ]]; then
        ...
        4|6)
            ...
            ;;
        *)
            echo "${FUNCNAME[0]}: the second parameter should be [4|6] (or missing for both)" >&2
            return 2
            ;;
        esac
    fi
...
{
    if [[ "$#" -ne 1 ]]; then
        echo "${FUNCNAME[0]}: please provide a single IP address as input" >&2
        return 1
    fi
