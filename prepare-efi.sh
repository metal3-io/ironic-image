...
build_efi() {
    ...
    dnf install -y --forcearch="$ARCH" $PACKAGES $SHIM_PKG
...
# Build native architecture first to be sure to grab tools/dependencies for correct architecture
ARCH="${HOST_ARCH}"
build_efi

# Then other architectures
for ARCH in "${SUPPORTED_ARCH[@]}"; do
    # Skip native one as we already did it
