...
- Setup the CI for the new branch in the prow configuration.
  [Prior art](https://github.com/metal3-io/project-infra/pull/1364)

Create a development branch (e.g. `prepare-x.y`) from the newly created branch:

- Pin the constraints.
   [Prior art](https://github.com/metal3-io/ironic-image/pull/1031)

- Pin `IRONIC_SOURCE` to a specific SHA in the upstream release branch. It must
   be in format `ARG IRONIC_SOURCE=<sha> # <branch_name>` for Renovate bot to
   be able to update it automatically in the future.

- Pin `NGS_SOURCE` (networking-generic-switch) to a specific SHA in the same
   way: `ARG NGS_SOURCE=<sha> # <branch_name>`.
...
Once the CI configuration and prepare-release PRs are merged,
...
[Prior art](https://github.com/metal3-io/ironic-standalone-operator/pull/722)
...
- The IrSO checkout `ref` in `.github/workflows/irso-functional.yml` to point
  to the matching IrSO release branch (e.g. `release-0.9`).
- `BMO_VERSION` in `.github/workflows/bmo-e2e.yml` to point to the latest
  stable BMO release compatible with the new ironic-image version.
...
- Commit your changes, push the new branch and create a pull request:
   ...
   - **Important!** The pull request must target `main`. The release workflow
    triggers on push to `main` when a file in `releasenotes/` changes.
...
Verify all release artifacts are correctly built or generated by the release
workflow. The following should exist:

Git tag pushed:

- Primary release tag: `vx.y.z`
...
[Prior art](https://github.com/metal3-io/ironic-image/pull/1068)
...
- Add the new release branch (e.g., `release-37.0`) to the `baseBranchPatterns`
  array
- Add the new release branch to the `packageRules` section matching the daily
  schedule of other release branches (`"* 0-5 * * *"`)
- Remove End-of-Life release branches from both `baseBranchPatterns` and
  `packageRules` to avoid unnecessary PR creation
